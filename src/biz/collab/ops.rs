...
fn patch_old_workspace_folder(
  ...
  let encoded_update = {
    ...
    let view = NestedChildViewBuilder::new(user.uid, workspace_id.to_string())
    ...
    for view_id in child_view_id_without_space {
      folder
        .body
        .move_nested_view(&mut txn, view_id, &space_id, None);
    }
